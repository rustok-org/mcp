...
| installer URLs pinned by tag (the same docs plus `README.md` and `DISCLAIMER.md`) | 1 | `tests/test_docs_one_command.py` — a **different** token from the row above, and until 0.12.0 its `INSTALL_TAG` was typed by hand, which let a release that forgot both the docs and the constant stay green. **Scope, and it differs from the row above:** the image form is swept across the whole tree since this circle, this one is not — a tag cannot be told from a historical reference without meaning. Why, in full: the docstring of `test_no_image_reference_escapes_the_registry` |
