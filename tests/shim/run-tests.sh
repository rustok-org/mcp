...
EXPECTED="claude mcp add -s user rustok -- podman run -i --rm --label rustok=wallet --label rustok.agent=claude -v rustok-wallet-tui:/data --secret rustok-keyring-claude,type=mount,mode=0400,uid=1000,gid=1000 -e RUSTOK_KEYRING_PASSWORD_FILE=/run/secrets/rustok-keyring-claude --secret rustok-rpc-claude-1,type=env,target=RUSTOK_RPC_URLS_1 -e RUSTOK_ALLOWED_CHAINS=1 ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7"
if assert_exit 0 \
...
unset RUSTOK_RPC_URLS_1 RUSTOK_ALLOWED_CHAINS
EXPECTED_JSON='{"command":"podman","args":["run","-i","--rm","--label","rustok=wallet","--label","rustok.agent=openclaw","-v","rustok-openclaw:/data","--secret","rustok-keyring-openclaw,type=mount,mode=0400,uid=1000,gid=1000","-e","RUSTOK_KEYRING_PASSWORD_FILE=/run/secrets/rustok-keyring-openclaw","--secret","rustok-rpc-openclaw-1,type=env,target=RUSTOK_RPC_URLS_1","-e","RUSTOK_ALLOWED_CHAINS=1","ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7"]}'
if assert_exit 0 \
...
CEXP='{"args":["run","-i","--rm","--label","rustok=wallet","--label","rustok.agent=cursor","-v","rustok-cursor:/data","--secret","rustok-keyring-cursor,type=mount,mode=0400,uid=1000,gid=1000","-e","RUSTOK_KEYRING_PASSWORD_FILE=/run/secrets/rustok-keyring-cursor","--secret","rustok-rpc-cursor-1,type=env,target=RUSTOK_RPC_URLS_1","-e","RUSTOK_ALLOWED_CHAINS=1","ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7"],"command":"podman"}'
...
OEXP='{"args":["run","-i","--rm","--label","rustok=wallet","--label","rustok.agent=claude","-v","rustok-wallet-tui:/data","--secret","rustok-keyring-claude,type=mount,mode=0400,uid=1000,gid=1000","-e","RUSTOK_KEYRING_PASSWORD_FILE=/run/secrets/rustok-keyring-claude","--secret","rustok-rpc-claude-1,type=env,target=RUSTOK_RPC_URLS_1","-e","RUSTOK_ALLOWED_CHAINS=1","ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7"],"command":"podman"}'
...
run_shim update
CJSON="$(jq -cS '.mcpServers.rustok' "$WORK/home/.cursor/mcp.json" 2>/dev/null || echo none)"
CEXP='{"args":["run","-i","--rm","--label","rustok=wallet","--label","rustok.agent=cursor","-v","rustok-cursor:/data","--secret","rustok-keyring-cursor,type=mount,mode=0400,uid=1000,gid=1000","-e","RUSTOK_KEYRING_PASSWORD_FILE=/run/secrets/rustok-keyring-cursor","ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7"],"command":"podman"}'
HCHECK="$("$PY3" - "$WORK/home/.hermes/config.yaml" <<'PEOF'
import sys, yaml
c = yaml.safe_load(open(sys.argv[1]))
r = c["mcp_servers"]["rustok"]
...
if assert_exit 0 && [ "$CJSON" = "$CEXP" ] && [ "$HCHECK" = "HOK" ] \
    && grep -q '^podman pull ghcr.io/rustok-org/rustok-wallet-tui:v0.9.7' "$WORK/log" \
    && [ "$PULL_BEFORE_ADD" = "1" ] \
