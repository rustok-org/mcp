...
`Dockerfile.wallet` pins the core (and, on this line, the console) it copies
binaries out of. These are **dependency pins**, not our own version — which is
...
* **NOT checked here — staleness.** Whether `v0.3.2` is still the current core
  ...
  network-bound. `v0.3.1` against a released `v0.3.2` passes this file. That
