...
> only (`v0.9.6`, `v0.9`, `v0`), on purpose: the installer pins the exact digest
...
```bash
curl --proto '=https' --tlsv1.2 -fsSL \
  https://raw.githubusercontent.com/rustok-org/mcp/wallet-tui-v0.9.6/scripts/install.sh -o install.sh
...
podman run -it --rm \
  ...
  ghcr.io/rustok-org/rustok-wallet-tui:v0.9.6 create-wallet
```
  ...
  "mcpServers": {
    "rustok": {
      ...
      "args": ["run", "-i", "--rm",
               ...
               "ghcr.io/rustok-org/rustok-wallet-tui:v0.9.6"]
    }
  }
}
```
...
```bash
docker pull ghcr.io/rustok-org/rustok-wallet-tui:v0.9.6
