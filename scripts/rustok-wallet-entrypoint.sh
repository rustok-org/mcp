...
#
# It used to reach it as an environment variable. Measured on the published
# v0.4.3: `tini` (PID 1) and `core-server` both carried it in /proc/<pid>/environ,
# readable by anything running as the same user. The gateway and the MCP server
# were already clean; these two were not, and the comment that used to sit here
# said so honestly — "closing that requires a change inside core". Core v0.1.4 is
# that change: it reads the password from a file itself.
#
...
if [ -n "${RUSTOK_KEYRING_PASSWORD:-}" ]; then
    ...
    # (`docker inspect`) and, under `--init`, in its own PID 1 — which is why
    ...
    if [ ! -f "$RUSTOK_KEYRING_PASSWORD_FILE" ] \
        ...
        echo "rustok-wallet: RUSTOK_KEYRING_PASSWORD_FILE does not point to a readable regular file: $RUSTOK_KEYRING_PASSWORD_FILE" 1>&2
        exit 1
    fi
    ...
    if [ -z "$probe" ]; then
        echo "rustok-wallet: RUSTOK_KEYRING_PASSWORD_FILE is empty: $RUSTOK_KEYRING_PASSWORD_FILE" 1>&2
        exit 1
    fi
    probe=""
...
# around, and `podman cp` reaches it there.
