...
### Ратификация Капитана по PR-1.1 (2026-07-16, эскалация #1 ревью)

Требование тест-плана «проба ОБА движка» удовлетворено для PR-1.1 как: **podman-live
e2e** (6 сценариев, включая negative/inspect) + **честные докстринги** (docker-фолбэк
помечен «manually probed only — NOT covered by this suite») + документированная ручная
docker-проба. Механизм чтения файла в entrypoint движко-агностичен. **Долг:** живой
docker-прогон при появлении docker-среды (машина или CI); тесты уже параметризованы
доставкой пароля (трекается в теле PR #70).
